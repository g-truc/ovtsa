...
glm::vec3 const & surface::get_texel(glm::uvec2 const& Position) const
{
...
void surface::add(glm::uvec2 const& Position, glm::vec3 const& Color)
{
	assert(glm::all(glm::lessThan(Position, this->Size)));
	this->Data[Position.x + Position.y * this->Size.x] += Color;
}

void surface::div(glm::uvec2 const& Position, float Value)
{
	assert(glm::all(glm::lessThan(Position, this->Size)));
	this->Data[Position.x + Position.y * this->Size.x] /= Value;
}

void surface::save_as(std::string const& Filename)
{
